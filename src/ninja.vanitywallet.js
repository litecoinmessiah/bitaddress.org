ninja.wallets.vanitywallet = {
	...
	generateKeyPair: function () {
		...
		var privateKey = key.getLitecoinHexFormat();
	...
	addKeys: function () {
		...
		var litecoinAddress = ninja.translator.get("vanityinvalidinputcouldnotcombinekeys");
		...
		try {
			...
			if (ninja.publicKey.isPublicKeyHexFormat(input1KeyString) && ninja.publicKey.isPublicKeyHexFormat(input2KeyString)) {
				...
				if (document.getElementById("vanityradioadd").checked) {
					...
					if (pubKeyByteArray == null) {
						...
						litecoinAddress = ninja.publicKey.getLitecoinAddressFromByteArray(pubKeyByteArray);
			...
			// one public key and one private key
				...
				var ecKey = (ninja.privateKey.isPrivateKey(input1KeyString)) ? new Litecoin.ECKey(input1KeyString) : new Litecoin.ECKey(input2KeyString);
				// add 
				if (document.getElementById("vanityradioadd").checked) {
				...
				if (pubKeyCombined == null) {
					...
					litecoinAddress = ninja.publicKey.getLitecoinAddressFromByteArray(pubKeyCombined);
			...
			// both inputs are private keys
				...
				if (combinedPrivateKey == null) {
					...
					litecoinAddress = combinedPrivateKey.getLitecoinAddress();
					privateKeyWif = combinedPrivateKey.getLitecoinWalletImportFormat();
		...
		document.getElementById("vanityaddress").innerHTML = litecoinAddress;
