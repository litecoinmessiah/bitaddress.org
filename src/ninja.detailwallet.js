ninja.wallets.detailwallet = {
		...
		if (ninja.privateKey.isBIP38Format(key)) {
			...
			ninja.privateKey.BIP38EncryptedKeyToByteArrayAsync(key, passphrase, function (btcKeyOrError) {
				...
				if (btcKeyOrError.message) {
					...
					ninja.wallets.detailwallet.populateKeyDetails(new Litecoin.ECKey(btcKeyOrError));
				}
			});
		}
		else {
			if (Litecoin.ECKey.isMiniFormat(key)) {
			...
			else if (Litecoin.ECKey.isBase6Format(key)) {
	...
	populateKeyDetails: function (btcKey) {
		if (btcKey.priv != null) {
			...
			var wif = btcKey.getLitecoinWalletImportFormat();
			document.getElementById("detailpubkey").innerHTML = btcKey.getPubKeyHex();
			document.getElementById("detailaddress").innerHTML = litecoinAddress;
			document.getElementById("detailprivwif").innerHTML = wif;
			btcKey.setCompressed(true);
			var litecoinAddressComp = btcKey.getLitecoinAddress();
			var wifComp = btcKey.getLitecoinWalletImportFormat();
			document.getElementById("detailpubkeycomp").innerHTML = btcKey.getPubKeyHex();
			document.getElementById("detailaddresscomp").innerHTML = litecoinAddressComp;
			document.getElementById("detailprivwifcomp").innerHTML = wifComp;

			ninja.qrCode.showQrCode({
				"detailqrcodepublic": litecoinAddress,
				"detailqrcodepubliccomp": litecoinAddressComp,
